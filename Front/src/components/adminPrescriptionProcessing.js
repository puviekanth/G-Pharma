...
useEffect(() => {
    axios.get(`${api}/PrescriptionOrdersGetProcessing`)
        .then(response => {
...
const handleOrderProcessing = (orderID) => {
    axios.put(`${api}/updateOrderStatus/${orderID}`,{status:'Completed',deliveryStatus:'Ready For Delivery'})
    .then(response =>{
    ...
    order.prescriptions.map((image, index) => {
        const imagePath = `${api}/${image.replace(/\\/g, '/')}`;
        return (
            <img
                key={index}
                src={imagePath}
