...
const AdminProductPage = () => {
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      if (editIndex !== null) {
        ...
        await axios.put(`${api}/updateproducts/${productId}`, formData, {
  ...
  const handleDeleteProduct = async (productID) => {
    try {
      await axios.delete(`${api}/deleteproducts/${productID}`);
      fetchProducts();
    } catch (error) {
              ...
              <div key={prod._id} className="product-item" style={{width:'40%',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center', marginRight:'20px'}}>
                ...
                {prod.image && (
                  <img 
                    ...
                    alt={prod.name} 
                    width="200px" 
                    onClick={() => handleImageClick(`${api}/${prod.image}`)} // Open modal on click
