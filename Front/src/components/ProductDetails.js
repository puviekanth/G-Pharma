...
function ProductDetail(){
    ...
    useEffect(() => {
        if (id) { // Check if the id is defined
            axios.get(`${api}/getindividualproduct?id=${id}`)
                .then(res => {
                    if (res.data.error) {
    ...
    useEffect(() => {
        if (id) { // Check if the id is defined
            axios.get(`${api}/getrelatedproduct`)
                .then(res => {
                    if (res.data.error) {
                        console.log(res.data.error);
                    } else {
...
const handlestockchange = (id, quantity) => {
    axios.put(`${api}/updatestock?id=${id}`, { quantity })
        .then(res => {
        ...
        <Navbar />
            ...
            <section className='prod-ind-page'>
              ...
              <div className='prod-ind-det-del' style={{marginTop:'30px'}}>
               ...
               <div className='img-det-ind'>
                    <img 
                            src={product.image ? `${api}/${product.image.replace(/\\/g, '/')}` : ''} 
                            alt='img' 
               ...
               <div className='del-info' style={{height:'56vh',marginTop:'28px'}}>
            ...
            <section className='related-products'>
                ...
                <div className='container'>
                        {randomProducts.map(prod => (
                             ...
                             <img src={`${api}/${prod.image.replace(/\\/g, '/')}`} alt={prod.name} className='pro-image'  onClick={() => navigatePage(prod._id)}/>
